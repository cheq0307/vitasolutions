...
@section('title', 'Mi Perfil de Salud')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-white">Mi Perfil de Salud</h1>
            <p class="text-slate-400 text-sm mt-1">Mantén tus datos actualizados para un seguimiento más preciso</p>
        </div>
        @if($perfil->updated_at && $perfil->updated_at != $perfil->created_at)
            <span class="text-xs text-slate-500 bg-slate-800 px-3 py-1 rounded-full">
                Actualizado {{ $perfil->updated_at->diffForHumans() }}
            </span>
        @endif
    </div>
    ...
    <form action="{{ route('cliente.perfil.update') }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
            <div class="flex items-center gap-3 mb-5">
                ...
                <div>
                    ...
                    <p class="text-slate-500 text-xs">Solo tu asesor puede ver esta información</p>
                </div>
            </div>
        ...
        {{-- SECCIÓN 3: OBJETIVO Y ACTIVIDAD --}}
        <div class="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
            <div class="flex items-center gap-3 mb-5">
        ...
        <div class="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h2 class="text-white font-semibold">Notas adicionales</h2>
            </div>
            <textarea name="notes" rows="3" placeholder="Cualquier información relevante para tu asesor..."
